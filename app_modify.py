...
# Modified function to read PDF and extract text
    ...
    for pdf in pdf_docs:
        try:
            ...
            for page in pdf_reader.pages:
                ...
                if page_text.strip():  # If text is extracted, use it
                    text += page_text
                else:  # If no text is extracted, assume it's an image-based PDF
                    images = convert_from_bytes(pdf.read())
                    for image in images:
                        text += pytesseract.image_to_string(image)
            if not text.strip():
                st.warning(f"No text could be extracted from '{pdf.name}'. The file might be empty or corrupted.")
...
def generate_full_summary(text, lang):
    ...
    Finally, strictly ensure to provide the summary in a clean formatted structure, headers, sub-headers, bullet points and even symbols, icons to express beautifully.
    """
